...
server.use("/cosmos/v1/drive/folder", async (req, res) => {
  const folderId = req.query.id;
  res.send(await getFiles(folderId));
});

server.use("/cosmos/v1/drive/file/:id", sendFileStream);

server.use("/cosmos/v1/drive/getAuthURL", (req, res) => {
...
server.use("/cosmos/v1/drive/getAuthToken", async (req, res) => {
  const CLIENT_ID = req.query.clientId;
  const CLIENT_SECRET = req.query.clientSecret;
...
server.use("/cosmos/v1/torrent", torrent);
server.use("/cosmos/v1/search", search);
server.use("/cosmos/v1/details", details);

server.get("/cosmos/v1/uptime", async (req, res) => {
  res.send({ uptime: humanTime(process.uptime() * 1000) });
});

server.get("/cosmos/v1/diskinfo", async (req, res) => {
  const path = req.query.path;
  const info = await diskinfo(path);
  res.send(info);
});

server.get("/cosmos/v1/status", async (req, res) => {
