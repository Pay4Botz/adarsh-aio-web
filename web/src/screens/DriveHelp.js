...
export default function DriveHelp() {
  ...
  const onSubmit = async e => {
    ...
    if (clientId && clientSecret && !authCode) {
      const resp = await fetch(`/cosmos/v1/drive/getAuthURL?clientId=${clientId}&clientSecret=${clientSecret}`).then(res => res.json());
      if (!resp || resp.error) {
      ...
      const resp = await fetch(
        `/cosmos/v1/drive/getAuthToken?clientId=${clientId}&clientSecret=${clientSecret}&authCode=${authCode}`
      ).then(res => res.json());
      if (!resp || resp.error) {
        setError(resp.error || "An error occured");
      } else {
